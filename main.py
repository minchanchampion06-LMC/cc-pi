...
class Entity:
    def __init__(self, x, y, tier_idx, is_bot=True):
        ...
        self.teeth_image = pygame.image.load("fat.io/wolf_teeth.png").convert_alpha()
        # 이빨 이미지 크기도 적절히 조절
    ...
    def use_wolf_bite(self, enemies):
        print(f"{self.name} 스킬 시도!")
        self.is_biting = True
        self.bite_timer = 0.5  # 0.5초 동안만 이빨이 보임 (매우 짧게)
        self.bite_cooldown = 3000

        # 공격 판정 위치 계산 (늑대 입 앞)
        dist = self.radius + 30
        bx = self.x + math.cos(self.angle) * dist
        by = self.y + math.sin(self.angle) * dist

        for victim in enemies:
            if victim == self: continue

            # 피격자와의 거리 계산
            d = math.sqrt((victim.x - bx) ** 2 + (victim.y - by) ** 2)
    ...
    def use_elephant_skill(self, enemies):
        if self.trunk_cooldown <= 0 and self.energy >= 50:
            self.is_trunk_swinging = True
            self.trunk_animation_timer = 0.5
            self.trunk_cooldown = 3500
            self.energy -= 50

            # 코끼리 울음소리나 휘두르는 소리가 있다면 여기서 재생
            elephant_sound.play()

            # 내 앞쪽 부채꼴 범위(또는 원형 범위)의 적 체크
            swing_range = self.radius + 330
            for victim in enemies:
                # 1. 거리 체크
                dist = math.hypot(self.x - victim.x, self.y - victim.y)
                if dist < swing_range:

                    # 2. [추가] 부채꼴 범위 체크 (내가 바라보는 방향 기준 약 120도 범위)
                    angle_to_victim = math.atan2(victim.y - self.y, victim.x - self.x)
                    # 내 각도와 상대 각도의 차이 계산
                    angle_diff = (angle_to_victim - self.angle + math.pi) % (2 * math.pi) - math.pi

                    # 약 60도(좌우 합 120도) 안에 있을 때만 공격 성공
                    if abs(angle_diff) < math.radians(60):
                        # 대미지 계산
                        total_damage = 20 + (victim.tier_idx * 15)
                        victim.hp -= total_damage

                        # 기절 상태 부여 (ms 단위로 3000 적용)
                        victim.stun_timer = 1000

                        # 넉백 처리 (강력하게 날려버림)
                        push_dir = math.atan2(victim.y - self.y, victim.x - self.x)
                        victim.knockback_speed = 1500
                        victim.knockback_angle = push_dir

            print(f"🐘 코끼리 코 휘두르기! (앞방향 범위 공격)")
    ...
    def draw(self, surface, cam_x, cam_y):
        ...
        # 1-2. 늑대 이빨 이미지
        if self.is_biting and self.tier_idx == 3:
            # 이빨 이미지 회전 (파이게임 각도는 반시계 방향이므로 -math.degrees)
            rotated_teeth = pygame.transform.rotate(self.teeth_image, -math.degrees(self.angle))

            # 입 앞 위치 계산
            dist = self.radius + 25
            bx = self.x + math.cos(self.angle) * dist - cam_x
            by = self.y + math.sin(self.angle) * dist - cam_y

            # 회전된 이미지의 중심을 bx, by에 맞춤
            rect = rotated_teeth.get_rect(center=(int(bx), int(by)))
            surface.blit(rotated_teeth, rect)
        ...
        if getattr(self, 'snowball_active', False):
            for i in [0, (2 / 3) * math.pi, (4 / 3) * math.pi]:  # 0도와 180도 (정반대 방향)
                ...
                # 여기에 self.angle을 더하지 마세요! 오직 snowball_angle만 사용합니다.
                snow_x = pos_x + math.cos(self.snowball_angle + i) * orbit_dist
                snow_y = pos_y + math.sin(self.snowball_angle + i) * orbit_dist

                snow_rect = self.snowball_img.get_rect(center=(int(snow_x), int(snow_y)))
                screen.blit(self.snowball_img, snow_rect)

        # 1-4. 코끼리 스윙 이미지
        if getattr(self, 'is_trunk_swinging', False):
            surface_size = int((self.radius + 180) * 2)
            gradient_surface = pygame.Surface((surface_size, surface_size), pygame.SRCALPHA)
            gradient_surface.fill((0, 0, 0, 0))

            center_g_x = surface_size // 2
            center_g_y = surface_size // 2
            swing_range_int = int(self.radius + 180)

            for r_offset in range(0, swing_range_int, 10):
                radius_to_draw = swing_range_int - r_offset

                # [진한 투명도 로직] 안쪽으로 갈수록 아주 진해짐
                alpha = int(100 + (155 * (r_offset / swing_range_int)))
                alpha = min(max(alpha, 0), 255)

                points = [(center_g_x, center_g_y)]
                for deg in range(-60, 61, 10):
                    rad = self.angle + math.radians(deg)
                    point_x = center_g_x + math.cos(rad) * radius_to_draw
                    point_y = center_g_y + math.sin(rad) * radius_to_draw
                    points.append((int(point_x), int(point_y)))

                if len(points) > 2:
                    # 색상 코드를 연두색(R:50, G:255, B:50)으로 변경!
                    # 더 부드러운 연두색을 원하면 (144, 238, 144, alpha)를 써보세요.
                    pygame.draw.polygon(gradient_surface, (50, 255, 50, alpha), points)
...
async def main():
    ...
    while True:
        ...
        # 플레이어 늑대 깨물기 타이머 관리
        if player.bite_cooldown > 0:
            player.bite_cooldown -= dt * 1000

        # 플레이어 북극곰 눈덩이 타이머 관리
        if player.tier_idx == 4:
            if player.snowball_active:
                player.snowball_timer -= dt
                # 눈덩이 회전 속도 (초당 5라디안씩 회전)
                player.snowball_angle += dt * 5

                if player.snowball_timer <= 0:
                    player.snowball_active = False

            if player.snowball_cooldown > 0:
        ...
        # 플레이어 코끼리 스윙 타이머 관리
        if player.tier_idx == 5:
            if player.trunk_cooldown > 0:
                player.trunk_cooldown -= dt * 1000
            if player.is_trunk_swinging:
                # 아주 잠깐(0.5초)만 보였다가 사라지게 함
                player.trunk_animation_timer -= dt
                if player.trunk_animation_timer <= 0:
                    player.is_trunk_swinging = False
        ...
        for event in events:
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    # 2단계 돼지 진흙탕 로직
                    if player.tier_idx == 1 and player.energy >= 50:
                        muds.append(Mud(player.x, player.y))
                        player.energy -= 50

                    # 3단계 멧돼지 박치기 로직
                    elif player.tier_idx == 2 and player.charge_cooldown <= 0 and player.energy >= 40:
                        player.is_charging = True
                        player.charge_timer = 2000  # 2초간 돌진
                        player.charge_cooldown = 3000  # 3초 쿨타임
                        player.energy -= 40

                    # 4단계 늑대 깨물기 로직
                    elif player.tier_idx == 3 and player.bite_cooldown <= 0:
                        player.use_wolf_bite(bots)  # 봇들을 공격 대상으로 전달
                        player.energy -= 30

                    # 5단계 북극곰 눈덩이 로직
                    elif player.tier_idx == 4:
                        player.use_polar_bear_skill()

                    # 6단계 코끼리 스윙 로직
                    elif player.tier_idx == 5:
                        player.use_elephant_skill([b for b in bots if b != player])
        ...
        if game_state == "playing":
            ...
            if player.stun_timer > 0:
                # 기절 중일 때는 현재 속도를 유지하되 AI 이동은 안 함 (관성용)
                pass
            elif player.tier_idx == 3 and player.is_biting:
                # 늑대 물기 중 (가장 느림)
            ...
            for ent in [player] + bots:
                if getattr(ent, 'is_biting', False):
                    ent.bite_timer -= dt
                    if ent.bite_timer <= 0:
                        ent.is_biting = False
